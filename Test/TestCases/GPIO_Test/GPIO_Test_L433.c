			...
			for(looper3 = Low; looper3 <= VeryHigh; looper3++)
			{
				for(looper4 = NoPull; looper4 <= PullDown; looper4++)
				{
					for(looper5 = PortA_0; looper5 <= PortH_15; looper5++)
					{
						...
						if((looper5 <= PortE_15) || ((looper >= PortH_0) && (looper <= PortH_15)))
						{
							int moder;
							int Otyper;
							int Ospeedr;
...
void GPIO_TestCase_2(void)
{
	...
	for(looper = Clear; looper <= Toggle; looper++)
	{
		...
		MemClear(&TestGPIOE, sizeof(TestGPIOE));
		MemClear(&TestGPIOH, sizeof(TestGPIOH));
		for(looper5 = PortA_0; looper5 <= PortH_15; looper5++)
		{
			if((looper5 <= PortE_15) || ((looper >= PortH_0) && (looper <= PortH_15)))
			{
	...
	for(looper = Clear; looper <= Toggle; looper++)
	{
		...
		TestGPIOE.ODR.Word = 0xFFFFFFFF;
		TestGPIOH.ODR.Word = 0xFFFFFFFF;
		for(looper5 = PortA_0; looper5 <= PortH_15; looper5++)
		{
			if((looper5 <= PortE_15) || ((looper >= PortH_0) && (looper <= PortH_15)))
			{
